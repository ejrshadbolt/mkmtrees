...
export default function EditPostPage({ params }: EditPostPageProps) {
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const postData = await response.json() as PostWithTags;
        setPost(postData);
        setFormData({
          ...
          author_id: postData.author_id_new || null,
        });
        
        // Fetch featured image URL if it exists
        if (postData.featured_image_id) {
          const mediaResponse = await fetch(`/api/admin/media/${postData.featured_image_id}`);
          if (mediaResponse.ok) {
            const mediaData = await mediaResponse.json() as { data: MediaItem };
            setFeaturedImageUrl(mediaData.data.url);
          }
        } else {
  ...
  }, [status, router, fetchPost, postId]);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600">
                Last updated {format(new Date(post.updated_at), "PPP")} by {post.author_name}
              </p>
            </div>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500">
                Control whether this post is visible to the public.
              </p>
            </div>
          </div>
